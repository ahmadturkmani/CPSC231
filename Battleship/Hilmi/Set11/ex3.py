...
def add_list(name, line):
	...
	mid = int(line/2)
	mid_len = int(mid - (mid/2))
	top = int(line)
	bot = int(line - line) #0
	mid_top = int(mid + mid_len)
	mid_bot = int(mid - mid_len)
	
	
	part_li = input("Which part of the list do you wish to see([u]pper, [l]ower, [m]iddle): ")
	
	if part_li[0].lower() == 'u':
		print(li[mid:top])
	elif part_li[0].lower() == 'm':
		print(li[mid_bot:mid_top])
	elif part_li[0].lower() == 'l':
		print(li[bot:mid])
	else:
		print("You have entered an invalid character.")

def do_quit():
	quit = input("Would you like to continue([Y]es, [N]o)? ")
